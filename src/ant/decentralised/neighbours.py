...
class RecirpocateAgent(BaseAgent):
    def allocate(self, time: int) -> np.ndarray:
        ...
        weighted_proportional_received = (self.received / production_vector) * self.market.resource_values

        fractions = weighted_proportional_received / np.sum(weighted_proportional_received)
...
class MaxFinder(BaseAgent):
    def allocate(self, time: int) -> np.ndarray:
        ...
        """
        weighted_production_vector = self.market.production_vector_calculation(time-1) * self.market.resource_values
        best_neighbour = self.edges()[np.argmax(weighted_production_vector[self.edges()])]
        allocation_vector = np.zeros(len(self.market))
