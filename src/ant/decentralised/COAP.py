...
def make_fixed_agent_coap_solver(n, i, community_indices, neighbours, endowments, resource_values):
    ...
    community_members_in_neighbourhood = community_array[community_members_in_neighbourhood_mask]
    
    # Map market ids to local ids
    community_local_idx = np.array([neighbour_to_local_idx[c] for c in community_members_in_neighbourhood])
    ...
    community_weights = cp.Constant(endowments[community_members_in_neighbourhood] * resource_values[community_members_in_neighbourhood])
    community_resource_values = cp.Constant(resource_values[community_members_in_neighbourhood])
    
    objective = cp.sum(
        cp.multiply(
            community_weights,
            cp.log(static_val_param + cp.multiply(x_i_neighbours[community_local_idx], community_resource_values) + SOLVER_EPSILON)
        )
    )
    ...
    def solve(X):
        ...
        received_utility_neighbours = received_utility[community_members_in_neighbourhood]
